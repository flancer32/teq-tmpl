...
| Nunjucks includes use the unlocalized loader without a default locale and use an explicitly configured default after the requested locale. | `test/unit/Back/Factory/Nunjucks/Env.test.js`, `test/integration/Container.test.mjs` with real cfg, and `test/integration/NunjucksLoaders.test.mjs` with real Nunjucks rendering. | The integration tests cover file includes, not every Nunjucks include variant. |
...
`test/integration/NunjucksLoaders.test.mjs` uses real Nunjucks includes and temporary files to check the unlocalized loader without a default locale and the explicit default fallback.
